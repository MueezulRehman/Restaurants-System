...
@section('content')
    <div class="max-w-xl mx-auto px-4 py-8">
        ...
        <div class="menu-card p-6 text-center">
            ...
            @else
                <p id="status-label" class="text-lg font-display font-semibold text-hut-green mb-1">{{ $order->status_label }}
                </p>
                ...
                <div class="w-full bg-gray-100 rounded-full h-3 overflow-hidden mb-2">
                    <div id="progress-bar" class="bg-hut-green h-3 rounded-full transition-all duration-700 ease-out"
                ...
                <div class="flex justify-between text-[10px] text-gray-400 mb-6">
                    ...
                    @foreach($steps as $key => $label)
                        <span
                            class="{{ array_search($order->status, array_keys($steps)) >= $loop->index ? 'text-hut-green font-medium' : '' }}">{{ $label }}</span>
                    @endforeach
                </div>
            @endif
                ...
                @foreach($order->items as $item)
                    <div class="flex justify-between text-sm py-1">
                        <span>{{ $item->quantity }}x {{ $item->item_name }}
                            {{ $item->size_label ? "({$item->size_label})" : '' }}</span>
    ...
    @push('scripts')
        ...
        @if(session('success'))
            <script>
                // Clear client-side cart after successful order placement
                try {
