...
@section('content')
    <div class="max-w-2xl mx-auto px-4 py-8">
        ...
        @if(!$customer)
            <div
                class="bg-hut-green/10 border border-hut-green/30 rounded-lg p-3 mb-4 flex items-center justify-between gap-3 flex-wrap">
                ...
                <div class="flex gap-2 shrink-0">
                    ...
                    <a href="{{ route('customer.register') }}" class="text-sm font-medium text-hut-green hover:underline">Create
                        account</a>
                </div>
            </div>
        @endif
            ...
            <div class="menu-card p-4 mb-4 space-y-4">
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2 mt-1.5">
                        @foreach(['dine_in' => 'Dine-in', 'takeaway' => 'Takeaway', 'delivery' => 'Home Delivery', 'online' => 'Online', 'table' => 'Table Order'] as $val => $label)
                            <label
                                class="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2 cursor-pointer has-[:checked]:border-hut-green has-[:checked]:bg-hut-green/5">
                ...
                <div>
                    ...
                    <input type="text" name="customer_name" required
                        ...
                        value="{{ old('customer_name', optional($customer)->name) }}">
                </div>
                ...
                <div>
                    ...
                    <input type="tel" name="customer_phone" required
                        class="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green"
                        value="{{ old('customer_phone', optional($customer)->phone) }}" placeholder="03XX-XXXXXXX">
                </div>

                @if(!$customer && session('otp_required'))
                    <div class="rounded-lg border border-hut-green/30 bg-hut-green/5 p-3 space-y-3">
                        <div>
                            <label class="text-sm font-medium text-gray-700">Verification delivery</label>
                            <select name="otp_channel" class="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2">
                                <option value="sms" {{ old('otp_channel', 'sms') === 'sms' ? 'selected' : '' }}>SMS</option>
                                <option value="whatsapp" {{ old('otp_channel') === 'whatsapp' ? 'selected' : '' }}>WhatsApp
                                </option>
                                <option value="both" {{ old('otp_channel') === 'both' ? 'selected' : '' }}>SMS and WhatsApp
                                </option>
                            </select>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-gray-700">Enter verification code</label>
                            <input type="text" name="otp" inputmode="numeric" maxlength="6" autocomplete="one-time-code"
                                class="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 tracking-[0.3em]"
                                placeholder="6-digit code" value="{{ old('otp') }}">
                            <p class="text-xs text-gray-500 mt-1">The code is linked to {{ $restaurant->name }} and expires in 5
                                minutes.</p>
                            <button type="submit" name="otp_resend" value="1"
                                class="text-sm font-semibold text-hut-green hover:underline">
                                Send code again
                            </button>
                        </div>
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            const savedOrderMode = localStorage.getItem('th_order_mode');
            if (savedOrderMode) {
                const savedMode = document.querySelector(`input[name="order_type"][value="${savedOrderMode}"]`);
                if (savedMode) {
            ...
            // Clear the cart only after the order is actually created.
