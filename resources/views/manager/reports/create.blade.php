            ...
            <form action="{{ route('manager.reports.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}"
                        class="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" required>
                    @error('name')
                ...
                @if(($branches ?? collect())->isNotEmpty())
                    <div>
                        <label class="block text-sm font-semibold text-gray-700">Branch</label>
                        <select name="branch_id" class="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
                ...
                <div class="flex items-center gap-3">
                    <button type="submit"
                        class="inline-flex items-center rounded-lg bg-hut-dark px-5 py-2 text-sm font-semibold text-white hover:bg-gray-800">Generate
                        Report</button>
