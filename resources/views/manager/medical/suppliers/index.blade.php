...
@section('page-content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('manager.suppliers.create') }}"
                class="px-4 py-2 bg-hut-green text-white rounded-lg hover:bg-hut-green/90 font-medium">
                ➕ Add Supplier
            </a>
        </div>
                        ...
                        @foreach($suppliers as $supplier)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-3 text-sm text-gray-600">
                                    ...
                                    @if($supplier->email)
                                        <br><a href="mailto:{{ $supplier->email }}"
                                            class="text-hut-green hover:underline">{{ $supplier->email }}</a>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('manager.suppliers.edit', $supplier) }}"
                                            class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-hut-green hover:bg-hut-green/10 hover:text-hut-green/80"
                                            aria-label="Edit supplier" title="Edit supplier">
                                            <x-icons.edit class="h-4 w-4" />
                                        </a>
                                        <form action="{{ route('manager.suppliers.destroy', $supplier) }}" method="POST"
                                            class="inline" data-confirm="Delete this supplier?">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-red-600 hover:bg-red-50 hover:text-red-800"
                                                aria-label="Delete supplier" title="Delete supplier">
