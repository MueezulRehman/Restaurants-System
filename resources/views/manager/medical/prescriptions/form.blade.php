        ...
        <form method="POST" action="{{ $prescription->id ? route('manager.prescriptions.update', $prescription) : route('manager.prescriptions.store') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="visit_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-hut-green focus:border-transparent">
                        <option value="">Not linked to a visit</option>
                        @foreach($visits as $visit)
                            <option value="{{ $visit->id }}" {{ old('visit_id', $prescription->visit_id) == $visit->id ? 'selected' : '' }}>
                                #{{ $visit->id }} — {{ $visit->patient?->name ?: 'Patient' }} ({{ ucfirst(str_replace('_', ' ', $visit->status)) }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Patient record</label>
                    <select name="patient_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-hut-green focus:border-transparent">
                        <option value="">Select a patient...</option>
                        @foreach($patients as $patient)
                            <option value="{{ $patient->id }}" {{ old('patient_id', $prescription->patient_id) == $patient->id ? 'selected' : '' }}>{{ $patient->name }} ({{ $patient->patient_number }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Doctor record</label>
                    <select name="doctor_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-hut-green focus:border-transparent">
                        <option value="">Select a doctor...</option>
                        @foreach($doctors as $doctor)
                            <option value="{{ $doctor->id }}" {{ old('doctor_id', $prescription->doctor_id) == $doctor->id ? 'selected' : '' }}>{{ $doctor->name }}{{ $doctor->specialty ? ' — '.$doctor->specialty : '' }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div>
                <span class="block text-sm font-medium text-gray-700 mb-2">Medicines</span>
                <div class="grid md:grid-cols-2 gap-2">
                    @php($selectedMedicines = old('medicines', $prescription->medicines ?: []))
                    @foreach($medicines as $medicine)
                        <label class="flex items-center gap-2 text-sm text-gray-700">
                            <input type="checkbox" name="medicine_ids[]" value="{{ $medicine->id }}" data-medicine-checkbox class="rounded border-gray-300" {{ in_array($medicine->id, is_array($selectedMedicines) ? $selectedMedicines : [], true) ? 'checked' : '' }}>
                            {{ $medicine->name }}
                        </label>
                    @endforeach
                </div>
                <input type="hidden" name="medicines" data-medicines-json value="{{ is_array($selectedMedicines) ? json_encode($selectedMedicines) : $selectedMedicines }}">
                @error('medicines') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </div>
...
<script>
    document.querySelector('form').addEventListener('submit', function () {
        document.querySelector('[data-medicines-json]').value = JSON.stringify(
            Array.from(document.querySelectorAll('[data-medicine-checkbox]:checked')).map(function (checkbox) {
