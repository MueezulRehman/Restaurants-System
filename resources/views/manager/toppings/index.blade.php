                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex justify-end gap-3">
                                        <a href="{{ route('manager.toppings.edit', $topping) }}"
                                            ...
                                            aria-label="Edit topping" title="Edit topping"><x-icons.edit class="h-4 w-4" /></a>
                                        <form action="{{ route('manager.toppings.destroy', $topping) }}" method="POST"
                                            data-confirm="Delete this topping?">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-red-600 hover:bg-red-50 hover:text-red-700"
                                                aria-label="Delete topping" title="Delete topping"><x-icons.trash class="h-4 w-4" /></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
