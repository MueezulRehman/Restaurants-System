        ...
        @if(($branches ?? collect())->isNotEmpty())
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                <form method="GET" class="flex flex-col gap-3 md:flex-row md:items-center">
                    ...
                    <select name="branch_id" onchange="this.form.submit()" class="rounded-lg border border-gray-200 px-3 py-2 text-sm">
                        <option value="">All branches</option>
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" @selected(($selectedBranch?->id ?? null) == $branch->id)>{{ $branch->name }} ({{ $branch->code }})</option>
                        @endforeach
                    </select>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($items as $item)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-3">
                                    @php
                                        $itemBranchQty = $selectedBranch ? ($branchInventoryMap['menu_item'][$item->id] ?? $item->stock_quantity) : ($item->stock_quantity ?? 0);
                                    @endphp
                                    <span class="inline-block px-3 py-1 rounded-full font-semibold
                                                @if($item->track_stock && $itemBranchQty <= 0)
                                                    bg-red-100 text-red-800
                                                @elseif($item->track_stock && $itemBranchQty <= 5)
                                                    bg-yellow-100 text-yellow-800
                            ...
                            @if($item->variants->isNotEmpty())
                                @foreach($item->variants as $variant)
                                    <tr class="hover:bg-gray-50 bg-gray-50">
                                        ...
                                        <td class="px-6 py-3">
                                            @php
                                                $variantBranchQty = $selectedBranch ? ($branchInventoryMap['variant'][$variant->id] ?? $variant->quantity_available) : ($variant->quantity_available ?? 0);
                                            @endphp
                                            <span class="inline-block px-3 py-1 rounded-full font-semibold
                                                                @if($variantBranchQty <= 0)
                                                                    bg-red-100 text-red-800
                                                                @elseif($variantBranchQty <= 5)
                                                                    bg-yellow-100 text-yellow-800
                                                                @else
                                                                    bg-green-100 text-green-800
                                                                @endif">
