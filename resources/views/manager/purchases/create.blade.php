...
@section('page-content')
    <div class="max-w-4xl">
        <x-back-link href="{{ route('manager.purchases.index') }}" label="Back to Purchase Batches" />
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <form action="{{ route('manager.purchases.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="supplier_name" value="{{ old('supplier_name') }}"
                            ...
                            placeholder="Supplier name or vendor">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="medicine_id" required
                            ...
                            @foreach($medicines as $m)
                                <option value="{{ $m->id }}" {{ old('medicine_id') == $m->id ? 'selected' : '' }}>{{ $m->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <input type="text" name="batch_number" value="{{ old('batch_number') }}"
                            class="w-full rounded-lg border border-gray-200 px-3 py-2 focus:outline-none focus:border-hut-green"
                            placeholder="Batch ID or lot number">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="rack_number" value="{{ old('rack_number') }}"
                            class="w-full rounded-lg border border-gray-200 px-3 py-2 focus:outline-none focus:border-hut-green"
                ...
                <div class="flex gap-3 pt-3">
                    <button type="submit"
                        class="bg-hut-green text-white px-6 py-2 rounded-lg font-medium hover:bg-hut-green/90">Record
                        Batch</button>
