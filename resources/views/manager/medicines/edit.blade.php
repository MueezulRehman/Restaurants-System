...
@section('page-content')
    <div class="max-w-4xl">
        <x-back-link href="{{ route('manager.medicines.index') }}" label="Back to Medicines" />
            ...
            <form action="{{ route('manager.medicines.update', $medicine) }}" method="POST" class="space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ old('name', $medicine->name) }}"
                            class="w-full rounded-lg border border-gray-200 px-3 py-2 focus:outline-none focus:border-hut-green">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="image" value="{{ old('image', $medicine->image) }}"
                            class="w-full rounded-lg border border-gray-200 px-3 py-2 focus:outline-none focus:border-hut-green"
                            placeholder="Optional image URL">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-hut-dark mb-1">Tax (%)</label>
                        <input type="number" name="tax" value="{{ old('tax', $medicine->tax) }}" step="0.01" min="0"
                            class="w-full rounded-lg border border-gray-200 px-3 py-2 focus:outline-none focus:border-hut-green"
                    ...
                    <div>
                        ...
                        <input type="text" name="sku" value="{{ old('sku', $medicine->sku) }}"
                            class="w-full rounded-lg border border-gray-200 px-3 py-2 focus:outline-none focus:border-hut-green">
                    </div>
                    <div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="flex items-center gap-3">
                        <input type="checkbox" id="requires_prescription" name="requires_prescription" value="1" {{ old('requires_prescription', $medicine->requires_prescription) ? 'checked' : '' }}
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" id="track_stock" name="track_stock" value="1" {{ old('track_stock', $medicine->track_stock) ? 'checked' : '' }}
                            class="h-4 w-4 rounded border-gray-300 text-hut-green focus:ring-hut-green">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        class="w-full rounded-lg border border-gray-200 px-3 py-2 focus:outline-none focus:border-hut-green">{{ old('description', $medicine->description) }}</textarea>
                </div>

                <div class="flex gap-3 pt-3">
                    <button type="submit"
                        class="bg-hut-green text-white px-6 py-2 rounded-lg font-medium hover:bg-hut-green/90">Update
                        Medicine</button>
