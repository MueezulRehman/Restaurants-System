...
@section('page-content')
    <div class="space-y-4">
        <div>
            <x-back-link href="{{ route('manager.customers.index') }}" label="Back to Customers" class="mb-0" />
        ...
        <div class="grid gap-4 md:grid-cols-3">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <p class="mt-2 text-sm text-gray-600">
                    {{ $customer->last_reminder_at?->format('M d, Y H:i') ?? 'Not reminded yet' }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <div class="mt-3 flex flex-wrap gap-2 items-center">
                    ...
                    @endif
                    <form method="POST" action="{{ route('manager.customers.remind', $customer) }}">@csrf<button
                            type="submit"
                            class="rounded-lg bg-hut-yellow px-3 py-2 text-sm font-semibold text-hut-dark hover:bg-amber-400">Remind
                            (Email / WhatsApp)</button></form>
                    @if($customer->phone)
                        @php
                            $waPhone = preg_replace('/\D+/', '', $customer->phone);
                            if (strlen($waPhone) === 10) {
                                $waPhone = '92' . ltrim($waPhone, '0');
                    ...
                    <form method="POST" action="{{ route('manager.customers.payment', $customer) }}" class="flex gap-2">
                        @csrf
                        <input type="number" step="0.01" min="0" name="amount" placeholder="Payment"
                            class="w-28 rounded-lg border border-gray-200 px-2 py-2 text-sm" />
                        <button type="submit"
                            class="rounded-lg border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Record
                            payment</button>
                    </form>
                </div>
            </div>
                    ...
                    <div class="p-4 space-y-3">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-xs text-gray-500">Received Rs.
                                    {{ number_format((float) ($order->amount_received ?? 0), 2) }}</p>
                                <div class="mt-2 flex flex-wrap justify-end gap-2">
                                    ...
                                    @if($customer->email)
                                        <form method="POST"
                                            action="{{ route('manager.customers.receipt.email', [$customer, $order]) }}">
                                            @csrf
                                            <button type="submit"
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    @forelse($order->items as $item)
                                        <tr>
                                            ...
                                            <td class="px-3 py-2 text-right text-gray-700">Rs.
                                                {{ number_format((float) $item->total_price, 2) }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="3" class="px-3 py-3 text-center text-gray-400">No line items on this bill.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
