        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <form action="{{ route('manager.staff.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="staff_type"
                            ...
                            @foreach($staffTypes as $staffType)
                                <option value="{{ $staffType }}" @selected(old('staff_type') === $staffType)>
                                    {{ ucfirst($staffType) }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div id="module-access-panel"
                    class="{{ old('role') === 'manager' ? '' : 'hidden' }} border border-gray-200 rounded-lg p-4 bg-gray-50">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            @foreach($modules as $module)
                                <label
                                    ...
                                    <input type="checkbox" name="module_access[]" value="{{ $module->key }}" {{ in_array($module->key, old('module_access', [])) ? 'checked' : '' }}
                ...
                <div class="flex gap-3 pt-4">
                    <button type="submit"
                        class="bg-hut-green text-white px-6 py-2 rounded-lg font-medium hover:bg-hut-green/90">Add Staff
                        Member</button>
