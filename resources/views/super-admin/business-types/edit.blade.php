    ...
    <div class="max-w-2xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <form action="{{ route('admin.business-types.update', $businessType) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $businessType->name) }}" required
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2">{{ old('description', $businessType->description) }}</textarea>
            </div>

            <div>
                <label class="mb-2 block text-sm font-medium text-gray-700">Icon</label>
                <input type="text" name="icon" value="{{ old('icon', $businessType->icon) }}"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2" />
            </div>
                ...
                <div class="grid gap-2 sm:grid-cols-2">
                    @foreach($modules ?? [] as $module)
                        <label class="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2">
                            <input type="checkbox" name="modules[]" value="{{ $module->id }}" {{ in_array($module->id, $selectedModules ?? []) ? 'checked' : '' }}
                                class="h-4 w-4 rounded border-gray-300 text-hut-dark" />
            ...
            <div class="flex items-center gap-3">
                <button type="submit"
                    class="rounded-lg bg-hut-dark px-4 py-2 font-semibold text-white hover:bg-gray-800">Update Business
                    Type</button>
                <x-back-link href="{{ route('admin.business-types.index') }}" label="Back to Business Types" class="mb-0" />
            </div>
        </form>
    </div>
@endsection