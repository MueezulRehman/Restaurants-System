    ...
    <div class="space-y-6">
        <div class="max-w-3xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xl tracking-wide text-amber-500">
                        {{ $feedback->rating ? str_repeat('★', $feedback->rating) : 'No rating provided' }}</p>
                </div>
                <div>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <form action="{{ route($feedbackPrefix . '.feedback.update-status', $feedback) }}" method="POST"
                    ...
                    <select name="status" class="w-full rounded-lg border border-gray-300 px-3 py-2">
                        ...
                        <option value="in_progress" {{ $feedback->status === 'in_progress' ? 'selected' : '' }}>In progress
                        </option>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <form action="{{ route($feedbackPrefix . '.feedback.reply', $feedback) }}" method="POST"
                    class="mt-4 space-y-4">
                    @csrf
                    <textarea name="message" rows="4" class="w-full rounded-lg border border-gray-300 px-3 py-2"
                        placeholder="Write a reply to the customer or manager"></textarea>
                    <label class="flex items-center gap-2 text-sm text-gray-700">
                        <input type="checkbox" name="is_internal" value="1" class="rounded border-gray-300" />
                        Internal note only
                    </label>
                    <button type="submit" class="rounded-lg bg-hut-dark px-4 py-2 font-semibold text-white">Send
                        reply</button>
                </form>
            </div>
        </div>
