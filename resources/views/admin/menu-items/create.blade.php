    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form action="{{ route('manager.menu-items.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="rounded-xl border border-hut-yellow/40 bg-hut-yellow/10 p-4 space-y-2">
                ...
                <p class="text-xs text-gray-600">Scan the product barcode first. The system will try to fill <strong>name</strong> and <strong>price</strong> automatically so you do not type every field by hand.</p>
                <label class="block text-sm font-medium text-hut-dark mb-1">Barcode</label>
                <input type="text" name="barcode" autofocus class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-base focus:outline-none focus:border-hut-green focus:ring-2 focus:ring-hut-green/20" value="{{ old('barcode') }}" placeholder="Scan or type barcode, then press Enter" id="product-barcode-input" autocomplete="off">
            </div>

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Item Name *</label>
                <input type="text" name="name" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('name') }}" placeholder="Auto-filled from barcode when possible">
            </div>
            ...
            <div>
                ...
                <input type="text" name="sku" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('sku') }}" placeholder="Optional — often same as barcode">
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="cost_price" step="0.01" min="0" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('cost_price') }}" placeholder="0">
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="unit" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('unit') }}" placeholder="e.g. piece, kg, bottle">
                </div>
