            ...
            <form action="{{ route('manager.categories.update', $category) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green">{{ old('description', $category->description) }}</textarea>
                </div>
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="icon"
                            class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green"
                            value="{{ old('icon', $category->icon) }}" placeholder="e.g. pizza">
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-2 pt-3">
                    <input type="checkbox" name="pos_show_line_edit" id="pos_show_line_edit" value="1" {{ old('pos_show_line_edit', $category->pos_show_line_edit) ? 'checked' : '' }} class="rounded">
                    <label for="pos_show_line_edit" class="text-sm text-hut-dark">Show weight/line-edit modal for items in
                        this category</label>
                </div>
                <p class="text-xs text-gray-500">Saving this setting also updates all existing items in this category.</p>
