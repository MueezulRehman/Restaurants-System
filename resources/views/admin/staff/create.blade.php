...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-lg font-display font-bold text-hut-dark mb-6">Add Staff Member</h2>
        ...
        <form action="{{ route('manager.staff.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('name') }}" placeholder="John Doe">
            </div>

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Email *</label>
                <input type="email" name="email" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('email') }}" placeholder="john@example.com">
            </div>

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Phone</label>
                <input type="tel" name="phone" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('phone') }}" placeholder="03XX-XXXXXXX">
            </div>
            ...
            <div id="module-access-panel" class="{{ old('role') === 'manager' ? '' : 'hidden' }} border border-gray-200 rounded-lg p-4 bg-gray-50">
                <label class="block text-sm font-medium text-hut-dark mb-1">Module Access</label>
                <p class="text-xs text-gray-500 mb-3">Pick which parts of the manager panel this manager is allowed to open and manage. You can grant only modules available to your own account.</p>

                @if($modules->isEmpty())
                    <p class="text-sm text-gray-500">No modules are enabled for this business yet. The Super Admin must enable a business module before it can be delegated.</p>
                @else
                    <div class="mb-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
                        <button type="button" onclick="applyPreset('all')" class="rounded-lg border border-hut-dark/30 bg-hut-dark/10 px-3 py-2 text-left text-sm font-medium text-hut-dark">All enabled</button>
                        ...
                        @endif
                        <button type="button" onclick="applyPreset('none')" class="rounded-lg border border-gray-300 bg-white px-3 py-2 text-left text-sm font-medium text-gray-600">Clear all</button>
                    </div>
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        ...
                        <label class="flex items-center gap-2 text-sm bg-white border border-gray-200 rounded-lg px-3 py-2 cursor-pointer hover:border-hut-green">
                            <input type="checkbox" name="module_access[]" value="{{ $module->key }}"
                                {{ in_array($module->key, old('module_access', [])) ? 'checked' : '' }}
            ...
            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Password *</label>
                <input type="password" name="password" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" placeholder="Minimum 8 characters">
                <p class="text-xs text-gray-500 mt-1">Share this password with the staff member securely.</p>
            </div>

            <div class="flex gap-3 pt-4">
                <button type="submit" class="bg-hut-green text-white px-6 py-2 rounded-lg font-medium hover:bg-hut-green/90">Add Staff Member</button>
