...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 class="text-lg font-display font-bold text-hut-dark mb-6">Edit Salary Record</h2>
        ...
        <form action="{{ route('manager.salary.update', $salary) }}" method="POST" class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <select name="user_id" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green">
                    ...
                    <option value="{{ $member->id }}" {{ old('user_id', $salary->user_id) == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" name="amount" step="0.01" min="0" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('amount', $salary->amount) }}">
            </div>

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Month *</label>
                <input type="month" name="month" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('month', $salary->month) }}">
            </div>

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Notes</label>
                <textarea name="notes" rows="2" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green">{{ old('notes', $salary->notes) }}</textarea>
            </div>
