            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
                <form method="POST" action="{{ route('manager.notifications.store') }}" class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" class="mt-1 w-full border rounded-lg px-3 py-2"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Title
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Type
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Status
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Channels
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Action
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($notifications as $notification)
                        ...
                        <tr>
                            <td class="px-4 py-3">
                                ...
                                @if($user->isSuperAdmin())
                                    <span
                                        class="block text-xs text-gray-500">{{ $notification->restaurant?->name ?? 'Platform' }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">{{ ucfirst(str_replace('_', ' ', $notification->type)) }}</td>
                            <td class="px-4 py-3">{{ ucfirst($notification->status) }}</td>
                            <td class="px-4 py-3">{{ implode(', ', $channels) }}</td>
                            <td class="px-4 py-3">
                                <form method="POST"
                                    action="{{ route($user->isSuperAdmin() ? 'admin.notifications.read' : 'manager.notifications.read', $notification) }}">
                                    @csrf
                                    <button class="px-3 py-1 rounded-lg bg-hut-yellow text-hut-dark font-medium">Mark
                                        read</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-6 text-center text-gray-500">No notifications found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
