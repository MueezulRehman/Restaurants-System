    ...
    <div class="mx-auto max-w-3xl">
        ...
        <form method="POST" action="{{ route('manager.purchasing.store') }}"
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div><label class="mb-1 block text-sm font-medium">Supplier name</label><input name="supplier_name"
                        class="w-full rounded-lg border border-gray-200 px-3 py-2" placeholder="Optional new supplier">
                </div>
                <div><label class="mb-1 block text-sm font-medium">Invoice number</label><input name="invoice_no"
            ...
            <div><label class="mb-1 block text-sm font-medium">Product</label><select name="menu_item_id" required
                    ...
                    <option value="{{ $item->id }}">{{ $item->name }} ({{ $item->sku ?? 'no SKU' }})</option>@endforeach
                </select></div>
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <div><label class="mb-1 block text-sm font-medium">Quantity</label><input type="number" name="quantity"
                        min="0.001" step="0.001" required class="w-full rounded-lg border border-gray-200 px-3 py-2"></div>
                ...
                <div><label class="mb-1 block text-sm font-medium">Selling price</label><input type="number"
                        name="selling_price" min="0" step="0.01" class="w-full rounded-lg border border-gray-200 px-3 py-2">
                </div>
            </div>
            <div><label class="mb-1 block text-sm font-medium">Notes</label><textarea name="notes" rows="3"
                ...
                class="rounded-lg bg-hut-dark px-5 py-2.5 text-sm font-semibold text-white">Receive stock</button>
        </form>
    </div>
@endsection