...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form action="{{ route('manager.menu-items.variants.update', [$item, $variant]) }}" method="POST" class="space-y-4">
            @csrf
            @method('PATCH')

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">SKU *</label>
                <input type="text" name="sku" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('sku', $variant->sku) }}">
            </div>

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Variant Name *</label>
                <input type="text" name="variant_name" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('variant_name', $variant->variant_name) }}">
            </div>

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Price Override</label>
                <input type="number" step="0.01" min="0" name="price_override" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('price_override', $variant->price_override) }}" placeholder="Leave blank to use base price">
            </div>
            ...
            <div class="flex gap-3 pt-4">
                <button type="submit" class="bg-hut-green text-white px-6 py-2 rounded-lg font-medium hover:bg-hut-green/90">Save Variant</button>
