...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('manager.menu-items.variants.create', $item) }}" class="bg-hut-green text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-hut-green/90">+ Add Variant</a>
</div>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3 text-right font-medium">{{ $variant->price_override !== null ? 'Rs. ' . number_format($variant->price_override) : 'Base price' }}</td>
                ...
                <td class="px-4 py-3 text-right space-x-2 flex justify-end">
                    <a href="{{ route('manager.menu-items.variants.edit', [$item, $variant]) }}" class="text-hut-green hover:underline text-xs font-medium">Edit</a>
                    <form action="{{ route('manager.menu-items.variants.destroy', [$item, $variant]) }}" method="POST" class="inline" onsubmit="return confirm('Delete this variant?')">
                        @csrf
                        @method('DELETE')
