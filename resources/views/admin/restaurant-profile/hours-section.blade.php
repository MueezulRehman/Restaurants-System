{{--
Include inside the Business Profile form (manager.restaurant.profile.edit).
...
<div class="rounded-xl border border-gray-200 bg-white p-5 space-y-4">
    <div>
        ...
        <p class="text-sm text-gray-500">Customers can only place online orders while you are open (unless you allow
            closed orders).</p>
    </div>
    ...
    <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 flex flex-wrap items-center gap-4">
        ...
        <input type="text" name="closed_message" value="{{ old('closed_message', $restaurant->closed_message) }}"
            placeholder="Message e.g. Closed for private event"
            class="flex-1 min-w-[200px] rounded-lg border border-amber-200 px-3 py-2 text-sm" />
    </div>
    ...
    <div class="space-y-2">
        @foreach($days as $day)
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 items-center rounded-lg border border-gray-100 px-3 py-2">
                ...
                <input type="time" name="opening_hours[{{ $day }}][open]" value="{{ $row['open'] ?? '09:00' }}"
                    class="rounded border px-2 py-1 text-sm" />
                <input type="time" name="opening_hours[{{ $day }}][close]" value="{{ $row['close'] ?? '22:00' }}"
                    class="rounded border px-2 py-1 text-sm" />
            </div>
        @endforeach
    </div>
