        ...
        <form action="{{ route('manager.attendance.update', $attendance) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="user_id" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green">
                    ...
                    <option value="{{ $member->id }}" {{ old('user_id', $attendance->user_id) == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="check_in" required class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('check_in', $attendance->check_in) }}">
                </div>

                <div>
                    <label class="block text-sm font-medium text-hut-dark mb-1">Check Out Time</label>
                    <input type="time" name="check_out" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green" value="{{ old('check_out', $attendance->check_out) }}">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-hut-dark mb-1">Notes</label>
                <textarea name="notes" rows="2" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-hut-green">{{ old('notes', $attendance->notes) }}</textarea>
            </div>
