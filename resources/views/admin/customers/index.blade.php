    ...
    <div class="space-y-4">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <form method="GET" class="flex gap-2">
                ...
                <button type="submit"
                    class="rounded-lg bg-hut-dark px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">Search</button>
            </form>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-linear-to-r from-hut-dark to-gray-800 p-4 text-white shadow-sm">
            <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-200">Create a profile so POS sales, partial payments, and account balances
                        stay organized.</p>
                </div>
                <form method="POST" action="{{ route('manager.customers.store') }}" class="grid gap-2 md:grid-cols-3">
                    ...
                    <input type="text" name="name" required placeholder="Customer name"
                        class="rounded-lg border border-white/20 bg-white/90 px-3 py-2 text-sm text-gray-800" />
                    <input type="text" name="phone" required placeholder="Phone"
                        class="rounded-lg border border-white/20 bg-white/90 px-3 py-2 text-sm text-gray-800" />
                    <input type="email" name="email" placeholder="Email (optional)"
                        class="rounded-lg border border-white/20 bg-white/90 px-3 py-2 text-sm text-gray-800" />
                    <input type="number" name="credit_limit" min="0" step="0.01" placeholder="Credit limit (optional)"
                        class="rounded-lg border border-white/20 bg-white/90 px-3 py-2 text-sm text-gray-800" />
                    <button type="submit"
                        class="md:col-span-4 rounded-lg bg-hut-yellow px-4 py-2 text-sm font-semibold text-hut-dark hover:bg-amber-400">Save
                        customer</button>
                </form>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-semibold text-gray-700">Credit limit</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($customers as $customer)
                            <tr>
                                ...
                                <td
                                    class="px-4 py-3 {{ $customer->balance > 0 ? 'text-red-600 font-semibold' : 'text-gray-600' }}">
                                    Rs. {{ number_format($customer->balance, 2) }}
                                    @if($customer->balance > 0)
                                        <span
                                            class="ml-1 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-bold uppercase text-red-700">Due</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-gray-600">
                                    {{ $customer->credit_limit !== null ? 'Rs. ' . number_format($customer->credit_limit, 2) : 'Unlimited' }}
                                </td>
                                <td class="px-4 py-3 text-gray-600">{{ $customer->last_reminder_at?->diffForHumans() ?? '—' }}
                                </td>
                                <td class="px-4 py-3 text-gray-600">{{ $customer->orders_count }}</td>
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('manager.customers.show', $customer) }}"
                                        class="text-hut-yellow hover:text-amber-600">View</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
