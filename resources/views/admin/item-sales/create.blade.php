...
@section('content')
    <div class="max-w-3xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p class="mb-6 text-sm text-gray-500">Select one or more items, then set a percent or fixed (Rs) discount. It will
            show live on the menu when active.</p>
        ...
        <form action="{{ route('manager.item-sales.store') }}" method="POST" class="space-y-5">
                    ...
                    @forelse($items as $item)
                        <label class="inline-flex items-center gap-2 text-sm">
                            <input type="checkbox" name="menu_item_ids[]" value="{{ $item->id }}"
                                class="form-checkbox h-4 w-4 text-hut-dark" {{ in_array($item->id, old('menu_item_ids', [])) ? 'checked' : '' }} />
                            <span>{{ $item->name }} <span class="text-gray-400">(Rs
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="label" value="{{ old('label', 'Sale') }}"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2" placeholder="Weekend Sale" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0.01" name="value" value="{{ old('value') }}" required
                        class="w-full rounded-lg border border-gray-300 px-3 py-2" placeholder="10 or 50" />
                    @error('value')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
