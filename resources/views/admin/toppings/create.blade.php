...
<div class="mx-auto max-w-2xl rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
    <div class="mb-6">
        ...
        <h2 class="mt-2 text-xl font-semibold text-hut-dark">Create Topping</h2>
    </div>

    <form action="{{ route('manager.toppings.store') }}" method="POST" class="space-y-4">
        @csrf
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <div>
                ...
                <input type="number" step="0.01" min="0" name="price" value="{{ old('price') }}" required class="w-full rounded-lg border border-gray-300 px-3 py-2" />
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Type</label>
                <input type="text" name="type" value="{{ old('type') }}" placeholder="e.g. cheese" class="w-full rounded-lg border border-gray-300 px-3 py-2" />
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="rounded-lg bg-hut-dark px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">Save Topping</button>
        </div>
    </form>
</div>
@endsection
