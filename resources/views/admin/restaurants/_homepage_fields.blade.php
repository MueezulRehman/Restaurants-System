...
{{-- @include('admin.restaurants._homepage_fields', ['restaurant' => $restaurant ?? null]) --}}
...
<div>
    ...
    <label class="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2">
        <input type="checkbox" name="show_on_homepage" value="1"
            {{ old('show_on_homepage', optional($restaurant ?? null)->show_on_homepage) ? 'checked' : '' }}
            class="form-checkbox h-4 w-4 text-hut-dark" />
        <span class="text-sm text-gray-700">Show on public homepage</span>
    </label>
    <p class="mt-1 text-xs text-gray-500">When enabled, this business appears in the main domain listing so customers can find and order from it.</p>
</div>
<div>
