            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2">
                @if($r)
                    @if(!empty($r->logo_path))
                        <img src="{{ asset('storage/' . $r->logo_path) }}" alt="{{ $r->name }}" class="w-10 h-10 rounded-full object-cover">
                    @else
                        <div class="w-10 h-10 bg-hut-yellow rounded-full flex items-center justify-center font-display font-bold text-hut-dark text-lg">{{ strtoupper(substr($r->name,0,1)) }}</div>
                    @endif
                    <div>
